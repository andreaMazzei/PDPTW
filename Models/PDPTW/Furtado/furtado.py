...
filename = "../../../Dataset/Instances/190_14.xlsx"
...
# Objective function
# model.setObjective(gp.quicksum(x[source, j]*(distance[source, j] + penalty) for j in set2) + x.prod(distance), GRB.MINIMIZE)
model.setObjective(x.prod(distance), GRB.MINIMIZE)
# model.addConstr(x.prod(distance) <= 159.1443+4*penalty)
# model.addConstr(gp.quicksum(x[source, j] for j in points_no_depots) <= 4)
...
model.addConstrs((x[i, j] == 1) >> (B[j] >= B[i] + service_time[i] + time[i, j]) for i in all_points for j in all_points if i != j and i != sink and j != source)
...
model.addConstrs(B[num_clients + i] >= B[i] + service_time[i] + time[i, num_clients + i] for i in pickup_points)
...
for route in routes:
    print(f"Route {route}")
    route_dist = route_distance(route, distance)
    total_distance += route_dist
    print(f"Route distance = {round(route_dist, 3)} km")
